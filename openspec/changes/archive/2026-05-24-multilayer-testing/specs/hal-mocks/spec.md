...
The test scaffold SHALL provide a `test_helpers.h` header with inline setters that write directly into the car module's internal state (via a `car_set_*` test-only API or by calling `car_process_frame` with crafted byte arrays). This allows output driver tests to set up known car state without needing a real or mocked CAN peripheral.
