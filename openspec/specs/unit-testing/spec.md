...
A set of test executables SHALL be built as part of the `CANMOD_TARGET=host` CMake configuration and registered with CTest. Running `ctest --test-dir build/host` SHALL execute all unit tests and report pass/fail with no hardware, no vcan0, and no root privileges required.
